...
入库时间：
    ...
    <div style="display:inline-block;width:49%;position: absolute;color: #fff;font-size: 28px;margin-left: 120px;line-height: 60px;margin-top: 120px;">
      <p>姓名:{{info.UserName}}</p>
      <p>性别:{{info.GenderType}}</p>
      <p>名族:{{info.Nation}}</p>
      <p>出生日期:{{info.Birthday}}</p>
      <p>身份证号:{{info.LICID}}</p>
      <img :src='info.Photo'  style="width: 120px;border: 1px inset #fff;height: 150px;position: absolute;top: 0%;left: 35%">
    </div>
    <RadioGroup v-model="out" @on-change="changeModel" style="color:white;;margin-top: 450px;position: absolute;margin-left: 400px;">
